...
class MLStrategy(bt.Strategy):
    ...
    def notify_trade(self, trade):
        if trade.isclosed:
            ...
            # Append trade details regardless of size
            self.deals.append({
            ...
            print(f"TRADE CLOSED: Type: {trade_type}, Buy: {buy_price}, Sell: {sell_price}, Open: {self.data.open[0]}, Close: {self.data.close[0]}, Profit: {profit}, Cash: {cash}")
        else:
    ...
    def next(self):
        ...
        if signal == Constants.BUYSIGNAL and not self.position:
            ...
            # Account for commission: Ensure cash > (size * price + commission)
            size = (self.broker.get_cash() * (1 - 0.001)) / close_price
            if size > 0:
                print(f"Executing BUY: Size: {size}, Price: {close_price}")
                self.buy(size=size)
        elif signal == Constants.SELLSIGNAL and self.position:
            print(f"Executing SELL: Size: {self.position.size}, Price: {self.data.close[0]}")
            self.sell(size=self.position.size)

    def stop(self):
        # Save completed trades to CSV
        output_file = "completed_trades.csv"
        keys = ['Trade Type', 'Buy Price', 'Sell Price', 'Open Price', 'Close Price', 'Profit', 'Cash After Trade']
        if not self.deals:
